...
const LostBooksList = () => {
  ...
  const { handleSort, sortData, getSortIndicator } = useSorting('loss_date', 'desc');
        ...
        <table className="data-table">
          <thead>
            <tr>
              ...
              <th>Читатель</th>
              <th onClick={() => handleSort('loss_date')} style={{cursor: 'pointer'}}>Дата утери{getSortIndicator('loss_date')}</th>
              <th onClick={() => handleSort('compensated')} style={{cursor: 'pointer'}}>Компенсация{getSortIndicator('compensated')}</th>
              <th onClick={() => handleSort('compensation_amount')} style={{cursor: 'pointer'}}>Сумма{getSortIndicator('compensation_amount')}</th>
              <th>Действия</th>
            </tr>
          </thead>
          <tbody>
            {sortData(lostBooks).length === 0 ? (
              <tr><td colSpan="6" className="no-data">Утерянных книг не найдено</td></tr>
            ) : (
              sortData(lostBooks).map(lost => (
                <tr key={lost.id}>
