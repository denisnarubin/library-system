-- =================================================
-- SQL-функции для нереализованных и частично реализованных запросов
-- =================================================

-- =================================================
-- ЗАПРОС №9 (НЕПОЛНОСТЬЮ РЕАЛИЗОВАН)
-- Новые и выбывшие читатели за период (месяц, семестр, год)
-- =================================================
...
BEGIN
    ...
    JOIN library_cards lc ON r.id = lc.reader_id
    LEFT JOIN students_details sd ON r.id = sd.reader_id
    WHERE lc.expiry_date >= start_date
...
-- =================================================
-- ЗАПРОС №11 (НЕРЕАЛИЗОВАН)
-- Наличие книги на абонементах и количество
-- =================================================
...
BEGIN
    ...
    JOIN publication_points pp ON bc.location_point_id = pp.id
...
-- =================================================
-- ЗАПРОС №12 (НЕРЕАЛИЗОВАН)
-- Читатели у которых на руках книга, кто раньше должен сдать
-- =================================================
...
-- =================================================
-- ЗАПРОС №13 (НЕРЕАЛИЗОВАН)
-- Полная информация о читателе по фамилии
-- =================================================
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(SUM(f.amount) FILTER (WHERE f.paid_date IS NULL), 0) AS unpaid_fines,
...
-- =================================================
-- ДОПОЛНИТЕЛЬНО: Улучшенная версия для запроса №10
-- Добавляет историю заказов за период
-- =================================================
